...
/**
 * A quadractic binary function (QBF_TP) is a function that can be expressed as the
 * sum of quadractic terms: f(x) = \sum{i,j}{a_{ij}*x_i*x_j}. In matricial form
 * a QBF_TP can be expressed as f(x) = x'.A.x 
 * The problem of minimizing a QBF_TP is NP-hard [1], even when no constraints
 * are considered.
 * 
 ...
 *
 */
public class QBF_TP implements Evaluator<Integer> {
	 ...
	 * 
	 * @param filename
	 *            Name of the file containing the input for setting the QBF_TP.
	 * @throws IOException
	...
	public QBF_TP(String filename) throws IOException {
	 ...
	 * is required to perform the matrix multiplication which defines a QBF_TP.
	 * 
	 * @param sol
	...
	/**
	 * {@inheritDoc} In the case of a QBF_TP, the evaluation correspond to
	 ...
	 * {@link #evaluateInsertionQBF_TP(int)}, {@link #evaluateRemovalQBF_TP(int)} and
	 * {@link #evaluateExchangeQBF_TP(int,int)}.
	 * 
	 * @return The evaluation of the QBF_TP.
	 */
	@Override
	public Double evaluate(Solution<Integer> sol) {

		setVariables(sol);
		return sol.cost = evaluateQBF_TP();

	}

	/**
	 * Evaluates a QBF_TP by calculating the matrix multiplication that defines the
	 * QBF_TP: f(x) = x'.A.x .
	 * 
	 * @return The value of the QBF_TP.
	 */
	public Double evaluateQBF_TP() {
	...
	public Double evaluateInsertionCost(Integer elem, Solution<Integer> sol) {
		...
		setVariables(sol);
		return evaluateInsertionQBF_TP(elem);

	}

	/**
	 * Determines the contribution to the QBF_TP objective function from the
	 ...
	 *         insertion.
	 */
	public Double evaluateInsertionQBF_TP(int i) {

		if (variables[i] == 1)
			return 0.0;

		return evaluateContributionQBF_TP(i);
	}
	...
	public Double evaluateRemovalCost(Integer elem, Solution<Integer> sol) {
		...
		setVariables(sol);
		return evaluateRemovalQBF_TP(elem);
	 ...
	 *         removal.
	 */
	public Double evaluateRemovalQBF_TP(int i) {
	...
	public Double evaluateExchangeCost(Integer elemIn, Integer elemOut, Solution<Integer> sol) {
		...
		setVariables(sol);
		return evaluateExchangeQBF_TP(elemIn, elemOut);
	 ...
	 *         exchange.
	 */
	public Double evaluateExchangeQBF_TP(int in, int out) {
		...
		if (in == out)
			return 0.0;
		if (variables[in] == 1)
			return evaluateRemovalQBF_TP(out);
		if (variables[out] == 0)
			return evaluateInsertionQBF_TP(in);

		sum += evaluateContributionQBF_TP(in);
		sum -= evaluateContributionQBF_TP(out);
	 ...
	 * solution. This method is different from {@link #evaluateInsertionQBF_TP(int)},
	 ...
	 *         insertion.
	 */
	private Double evaluateContributionQBF_TP(int i) {
	...
	/**
	 * Responsible for setting the QBF_TP function parameters by reading the
	...
	/**
	 * A main method for testing the QBF_TP class.
	 * 
	 * @param args
	 * @throws IOException
	 */
	public static void main(String[] args) throws IOException {
		...
		QBF_TP QBF_TP = new QBF_TP("matrix40");
		QBF_TP.printMatrix();
		...
		for (int i = 0; i < 10000; i++) {
			for (int j = 0; j < QBF_TP.size; j++) {
				if (Math.random() < 0.5)
					QBF_TP.variables[j] = 0.0;
				else
					QBF_TP.variables[j] = 1.0;
			}
			System.out.println("x = " + Arrays.toString(QBF_TP.variables));
			Double eval = QBF_TP.evaluateQBF_TP();
		...
		System.out.println("x = " + Arrays.toString(QBF_TP.variables));
		System.out.println("f(x) = " + QBF_TP.evaluateQBF_TP());
